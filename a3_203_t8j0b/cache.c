...
static cache_line_t *find_available_cache_line(cache_t *cache, cache_set_t *cache_set) {
  ...
  else if ((cache->policies & CACHE_REPLACEMENTPOLICY_MASK) == CACHE_REPLACEMENTPOLICY_RANDOM)
  {
    // get a random number between 0 and the number of lines in cache
    int index = rand() % cache->associativity;
    return cache_line_make_mru(cache_set, index);
  }
  // if no policy selected, use the first line
  else {
...
long cache_read(cache_t *cache, long *address) {
  ...
  /* TO BE COMPLETED BY THE STUDENT */
  long tag = (long) address >> cache->tag_shift;
  long offset = (long) address & cache->block_offset_mask;
  long index = (long) address >> cache->cache_index_shift & cache->cache_index_mask;
  cache_set_t *c_set = &(cache->sets[index]);

  cache->access_count++;

  cache_line_t *line = cache_set_find_matching_line(cache, c_set, tag);

  if (line == NULL)
  {
    cache->miss_count++;
    line = cache_set_add(cache, c_set, (uintptr_t) address, (uintptr_t) tag);
  }
